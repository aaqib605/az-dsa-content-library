...
<VIDEO_ID>3442</VIDEO_ID> <!-- Required -->
...
Imagine trying to find a specific word in a dictionary where the pages are shuffled randomly. It would be a nightmare! But because a dictionary is _sorted_ alphabetically, you can find any word in seconds.
...
_(Note: Under the hood, C++ uses **IntroSort**—a clever hybrid of QuickSort, HeapSort, and Insertion Sort. This guarantees an $O(N \log N)$ worst-case time complexity, making it safer than a naive QuickSort!)_
...
What if you want to sort something complex? For example, sorting a list of students by their _marks_, but if two students have the same marks, sort them by their _names_ alphabetically?
...
Imagine you sort a list of emails by their received date. Later, you sort that _same_ list by the sender's name.
