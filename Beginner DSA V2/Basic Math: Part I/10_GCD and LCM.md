...
<VIDEO_ID>3473</VIDEO_ID> <!-- Required -->
...
_Socratic Question:_ What if $A = 10^9$ and $B = 10^9 - 1$? The loop will run a billion times just to return 1! How do we do this efficiently?
...
> 💡 **CP Insight:** How fast is `gcd`? Under the hood, it uses an advanced mathematical technique called the _Euclidean Algorithm_ (which we will learn how to write from scratch later). For now, all you need to know is that it runs in **$O(\log(\min(A, B)))$** time! It is blazing fast and handles massive numbers instantly.
...
**The Fix:** Always divide _first_. Since the GCD is guaranteed to perfectly divide $A$, perform the division before the multiplication.
